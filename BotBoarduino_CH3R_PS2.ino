...
void SingleLegControl() {
    ...
    for (byte LegIndex = 0; LegIndex < 6; LegIndex++) if (g_Legs[LegIndex].posY != (short)pgm_read_word(&cInitPosY[LegIndex])) g_Hexapod.AllDown = false;
...
boolean TerminalMonitor(void) {
    static boolean g_fShowDebugPrompt = true;
