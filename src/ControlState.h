...
typedef struct _InControlState {
    ...
    uint8_t    GaitType;           // Current gait selection
    short   LegLiftHeight;      // Height legs lift during gait
    COORD3D TravelLength;       // X-Z movement, Y is rotation

    uint8_t    SelectedLeg;        // Currently selected leg for Single Leg Mode
    COORD3D SLLeg;              // Single leg position offset
    bool fSLHold;            // Single leg control mode hold flag

    bool BalanceMode;        // Global balance mode flag

    uint8_t    InputTimeDelay;     // Delay for "sneaking" effect based on input speed
    word    SpeedControl;       // Adjustable overall delay
    uint8_t    ForceGaitStepCnt;   // Force a step even when not moving
} INCONTROLSTATE;
